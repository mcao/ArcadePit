exports.run = async (bot, msg, args, level) => {
  ...
  if (bot.event.timed)
    return msg.reply("this race is in timed mode, not scored mode!");
  if (!msg.mentions.users.first())
    return msg.reply("please mention someone to change their score!");
  if (!bot.event.participants[msg.mentions.users.first().id])
    return msg.reply("the mentioned user is not in this race!");
  ...
  msg.reply(
    `${msg.mentions.users.first()}'s score has been changed to **${args[1]}**!`
  );
};
