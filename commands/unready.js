exports.run = async (bot, msg, args, level) => {
  ...
  if (!bot.openEvent.participants[msg.author.id])
    return msg.reply("you are not registered for this race!");
  if (!bot.openEvent.participants[msg.author.id].ready)
    return msg.reply("you are already marked as not ready!");

  bot.openEvent.participants[msg.author.id].ready = false;
  msg.reply(
    "you have been marked down as NOT ready for **" + bot.openEvent.name + "**!"
  );
};
