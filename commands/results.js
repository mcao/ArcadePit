...
exports.run = async (bot, msg, args, level) => {
  ...
  if (bot.openEvent && event.name == bot.openEvent.name) event = bot.openEvent;
  else if (bot.event && event.name == bot.event.name) event = bot.event;
  ...
  for (var key in event.participants) {
    if (event.participants[key].finished) {
      finished +=
        "<@" + key + "> (" + event.participants[key].time + " seconds)\n";
    } else if (
      !event.participants[key].finished &&
      event.participants[key].time != 0
    ) {
      dnf +=
        "<@" + key + "> (DNF, " + event.participants[key].time + " seconds)\n";
    } else if (
      !event.participants[key].finished &&
      event.participants[key].time == 0
    ) {
  ...
  if (finished != "") embed.addField("Finished", finished);
  if (dnf != "") embed.addField("Forfeited", dnf);
  if (stillPlaying != "")
